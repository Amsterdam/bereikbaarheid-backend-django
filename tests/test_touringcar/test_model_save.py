...
@pytest.fixture(scope="class")
def bericht_latlon():
    return baker.prepare(Bericht, geometry="SRID=28992;POINT (121278.2906402176 487150.7277812314)", lat=52.000000, lon=4.0000000, pk=1 )

@pytest.fixture(scope="class")
def bericht_geomdefault():
    return baker.prepare(Bericht, geometry=DEFAULT_GEOM, lat=52.371198, lon=4.8920418, pk=2 )

@pytest.fixture(scope="class")
def bericht_geom():
    return baker.prepare(Bericht, geometry="SRID=28992;POINT (121278.2906402176 487150.7277812314)", lat=None, lon=None, pk=3 )
...
class TestModelSave:
    ...
    def test_save_ifgeomdefault(self, bericht_geomdefault):
        ...
        assert str(Bericht.objects.get(pk=2).geometry) == "SRID=28992;POINT (121278.2906402176 487150.7277812314)"
